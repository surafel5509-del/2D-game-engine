...
data class EditorProject(
    ...
    fun seedDemoContent() {
        ...
        if (animations.isEmpty()) {
            animations += AnimationClip("Idle", listOf("player", "player"), 8f, PlaybackMode.LOOP)
            animations += AnimationClip("Run", listOf("player", "player", "player", "player"), 11f, PlaybackMode.LOOP)
        }
    }
}
