...
/** Production editor services shared by the in-app editor and play mode. */
...
data class RuntimeNode(
    val id: String = UUID.randomUUID().toString(), var name: String = "Node2D",
    var position: NovaVec2 = NovaVec2(), var rotation: Float = 0f, var scale: NovaVec2 = NovaVec2(1f,1f),
    var visible: Boolean = true, var zIndex: Int = 0, var parentId: String? = null,
    var texture: String = "", var script: String? = null,
    var bodyType: String = "NONE", var collider: String = "NONE",
...
class RuntimeScene(var name: String = "MainScene") {
    ...
    fun remove(id:String):RuntimeNode?{fun r(list:MutableList<RuntimeNode>):RuntimeNode?{val i=list.indexOfFirst{it.id==id};if(i>=0)return list.removeAt(i);list.forEach{r(it.children)?.let{return it}};return null};return r(roots)?.also{modified=true}}
}

object RuntimeSceneCodec {
    fun encode(scene:RuntimeScene):String {
        fun node(n:RuntimeNode):JSONObject=JSONObject().apply{put("id",n.id);put("name",n.name);put("x",n.position.x);put("y",n.position.y);put("rotation",n.rotation);put("sx",n.scale.x);put("sy",n.scale.y);put("visible",n.visible);put("z",n.zIndex);put("parent",n.parentId);put("texture",n.texture);put("script",n.script);put("body",n.bodyType);put("collider",n.collider);put("children",JSONArray(n.children.map(::node)))}
        return JSONObject().put("format",1).put("name",scene.name).put("roots",JSONArray(scene.roots.map(::node))).toString(2)
    }
    fun decode(raw:String):RuntimeScene {
        val o=JSONObject(raw);val s=RuntimeScene(o.optString("name","MainScene"))
        fun node(v:JSONObject):RuntimeNode{val n=RuntimeNode(v.optString("id",UUID.randomUUID().toString()),v.optString("name","Node2D"),NovaVec2(v.optDouble("x").toFloat(),v.optDouble("y").toFloat()),v.optDouble("rotation").toFloat(),NovaVec2(v.optDouble("sx",1.0).toFloat(),v.optDouble("sy",1.0).toFloat()),v.optBoolean("visible",true),v.optInt("z"),v.optString("parent","").ifEmpty{null},v.optString("texture"),v.optString("script","").ifEmpty{null},v.optString("body","NONE"),v.optString("collider","NONE"));val a=v.optJSONArray("children")?:JSONArray();for(i in 0 until a.length())n.children+=node(a.getJSONObject(i));return n}
        val a=o.optJSONArray("roots")?:JSONArray();for(i in 0 until a.length())s.roots+=node(a.getJSONObject(i));s.modified=false;return s
    }
}
...
/** Deterministic animation clock. Existing EditorModel.AnimationClip remains the public asset model. */
class NovaAnimationRuntime(private val clip:AnimationClip){var timeMs=0L;var playing=false;fun play(reset:Boolean=false){if(reset)timeMs=0;playing=true};fun stop(){playing=false;timeMs=0};fun update(dtMs:Long){if(!playing||clip.frames.isEmpty())return;timeMs+=dtMs;val total=clip.frames.sumOf{it.durationMs.toLong()};if(total>0&&clip.loop)timeMs%=total};fun frameIndex():Int{if(clip.frames.isEmpty())return 0;var t=if(clip.loop)timeMs else min(timeMs,(clip.frames.sumOf{it.durationMs.toLong()}-1).coerceAtLeast(0));for(i in clip.frames.indices){t-=clip.frames[i].durationMs;if(t<0)return i};return clip.frames.lastIndex}}
