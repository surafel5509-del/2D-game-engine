...
class V2TileMap(val width: Int = 64, val height: Int = 36, val layers: Int = 4) {
    ...
    fun paint(layer: Int, x: Int, y: Int, tile: Int, radius: Int = 0) { for (yy in y - radius..y + radius) for (xx in x - radius..x + radius) set(layer, xx, yy, tile) }
    fun fillRect(layer: Int, x0: Int, y0: Int, x1: Int, y1: Int, tile: Int) { for (y in minOf(y0, y1)..maxOf(y0, y1)) for (x in minOf(x0, x1)..maxOf(x0, x1)) set(layer, x, y, tile) }
    ...
    fun serialize(): String = JSONObject().put("format", 2).put("width", width).put("height", height).put("layers", layers).apply {
        val layerArray = JSONArray(); for (layer in 0 until layers) layerArray.put(JSONArray(cells[layer].toList())); put("cells", layerArray)
        val collisionArray = JSONArray(); for (layer in 0 until layers) collisionArray.put(JSONArray(collisions[layer].map { it })); put("collisions", collisionArray)
    }.toString()
...
class V2ProjectIO(private val context: Context) {
    ...
    fun allFiles(): List<File> = projectRoot.walkTopDown().filter { it.isFile }.toList()
    fun writeScene(scene: V2Scene): File = sceneFile(scene.name).also { file ->
        val root = JSONObject().put("format", 2).put("name", scene.name); val nodes = JSONArray()
    ...
    fun importFile(uri: android.net.Uri, displayName: String): File? = runCatching { val target = assetFile(displayName); context.contentResolver.openInputStream(uri)?.use { input -> FileOutputStream(target).use { output -> input.copyTo(output) } }; target }.getOrNull()
    fun exportBundle(output: File, scene: V2Scene, scriptsByName: Map<String, String>): File = output.also { target ->
        writeScene(scene); scriptsByName.forEach { (name, source) -> scriptFile(name).writeText(source) }
        ZipOutputStream(FileOutputStream(target)).use { zip -> allFiles().forEach { file -> val relative = projectRoot.toPath().relativize(file.toPath()).toString(); zip.putNextEntry(ZipEntry(relative)); file.inputStream().use { it.copyTo(zip) }; zip.closeEntry() } }
    }
}

data class V2BuildConfig(var applicationId: String = "com.nova.game", var versionName: String = "2.0.0", var versionCode: Int = 200, var minSdk: Int = 24, var targetSdk: Int = 35, var orientation: String = "landscape", var debug: Boolean = true, var aab: Boolean = false)
