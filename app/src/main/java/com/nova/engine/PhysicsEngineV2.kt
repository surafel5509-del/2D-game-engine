...
data class PhysicsV2Material(val density:Float=1f,val friction:Float=.7f,val restitution:Float=.05f) { fun clean()=copy(density.coerceAtLeast(.001f),friction.coerceIn(0f,1f),restitution.coerceIn(0f,1f)) }
data class PhysicsV2Filter(val categoryBits:Int=1,val maskBits:Int=-1,val groupIndex:Int=0) { fun allows(o:PhysicsV2Filter):Boolean { if(groupIndex!=0&&groupIndex==o.groupIndex)return groupIndex>0; return (maskBits and o.categoryBits)!=0&&(o.maskBits and categoryBits)!=0 } }
data class PhysicsV2Shape(val type:PhysicsV2ShapeType,val width:Float=1f,val height:Float=1f,val radius:Float=.5f,val length:Float=1f) { fun clean()=copy(width=width.coerceAtLeast(.001f),height=height.coerceAtLeast(.001f),radius=radius.coerceAtLeast(.001f),length=length.coerceAtLeast(0f)) }
data class PhysicsV2BodyDef(val id:Int,val type:PhysicsV2BodyType=PhysicsV2BodyType.DYNAMIC,val x:Float=0f,val y:Float=0f,val angle:Float=0f,val vx:Float=0f,val vy:Float=0f,val angularVelocity:Float=0f,val shape:PhysicsV2Shape=PhysicsV2Shape(PhysicsV2ShapeType.BOX),val material:PhysicsV2Material=PhysicsV2Material(),val filter:PhysicsV2Filter=PhysicsV2Filter(),val sensor:Boolean=false,val gravityScale:Float=1f,val linearDamping:Float=.02f,val angularDamping:Float=.02f,val fixedRotation:Boolean=false,val bullet:Boolean=false,val allowSleep:Boolean=true)
...
private class V2Body(def:PhysicsV2BodyDef) {
    val id=def.id; var type=def.type; val position=PhysicsV2Vec2(def.x,def.y); val velocity=PhysicsV2Vec2(def.vx,def.vy); var angle=def.angle; var angularVelocity=def.angularVelocity; var shape=def.shape.clean(); var material=def.material.clean(); var filter=def.filter; var sensor=def.sensor; var gravityScale=def.gravityScale; var linearDamping=def.linearDamping.coerceAtLeast(0f); var angularDamping=def.angularDamping.coerceAtLeast(0f); var fixedRotation=def.fixedRotation; var bullet=def.bullet; var allowSleep=def.allowSleep; var awake=true; var grounded=false; var sleepTimer=0f; var force=PhysicsV2Vec2(); var torque=0f; var mass=0f; var invMass=0f; var inertia=0f; var invInertia=0f
    init { recalculateMass() }
    fun dynamic()=type==PhysicsV2BodyType.DYNAMIC
    fun recalculateMass(){ if(!dynamic()){mass=0f;invMass=0f;inertia=0f;invInertia=0f;return}; val d=material.density; when(shape.type){ PhysicsV2ShapeType.BOX->{mass=d*shape.width*shape.height;inertia=mass*(shape.width*shape.width+shape.height*shape.height)/12f}; PhysicsV2ShapeType.CIRCLE->{mass=d*Math.PI.toFloat()*shape.radius*shape.radius;inertia=.5f*mass*shape.radius*shape.radius}; PhysicsV2ShapeType.CAPSULE->{val r=shape.radius;val l=shape.length;mass=d*(l*2f*r+Math.PI.toFloat()*r*r);inertia=mass*(l*l+4f*r*r)/12f} }; invMass=if(mass>0f)1f/mass else 0f;invInertia=if(!fixedRotation&&inertia>0f)1f/inertia else 0f }
    fun wake(){awake=true;sleepTimer=0f}
}
private class V2Joint(val def:PhysicsV2JointDef)

class PhysicsWorldV2(var fixedStep:Float=1f/60f,var velocityIterations:Int=8,var positionIterations:Int=3,var maxSubSteps:Int=8,var cellSize:Float=128f) {
    val gravity=PhysicsV2Vec2(0f,1600f); var timeScale=1f; var paused=false; var continuous=true; var allowSleeping=true; var sleepLinearThreshold=5f; var sleepAngularThreshold=.05f; var sleepTime=.5f
    private val bodies=LinkedHashMap<Int,V2Body>(); private val joints=LinkedHashMap<Int,V2Joint>(); private val contacts=ArrayList<PhysicsV2Contact>(); private val currentPairs=HashSet<Long>(); private val previousPairs=HashSet<Long>(); private var accumulator=0f; private var beginCount=0; private var stayCount=0; private var endCount=0
    private val beginListeners=ArrayList<(PhysicsV2Contact)->Unit>(); private val stayListeners=ArrayList<(PhysicsV2Contact)->Unit>(); private val endListeners=ArrayList<(Int,Int)->Unit>(); var simulationTime=0f;private set; var stepCount=0L;private set
    ...
    fun onBegin(listener:(PhysicsV2Contact)->Unit){beginListeners+=listener}; fun onStay(listener:(PhysicsV2Contact)->Unit){stayListeners+=listener}; fun onEnd(listener:(Int,Int)->Unit){endListeners+=listener}
    ...
    private fun tick(dt:Float){ previousPairs.clear();previousPairs.addAll(currentPairs);currentPairs.clear();contacts.clear(); bodies.values.forEach{b->b.wakeIfMoving();b.grounded=false;if(b.dynamic()&&b.awake){b.velocity.x+=(gravity.x*b.gravityScale+b.force.x*b.invMass)*dt;b.velocity.y+=(gravity.y*b.gravityScale+b.force.y*b.invMass)*dt;b.velocity.x*=1f/(1f+b.linearDamping*dt);b.velocity.y*=1f/(1f+b.linearDamping*dt);if(!b.fixedRotation)b.angularVelocity*=1f/(1f+b.angularDamping*dt);b.position.x+=b.velocity.x*dt;b.position.y+=b.velocity.y*dt;b.angle+=b.angularVelocity*dt;b.force.set(0f,0f);b.torque=0f}}; val list=bodies.values.toList();for(i in list.indices)for(j in i+1 until list.size){val a=list[i];val b=list[j];if(a.type==PhysicsV2BodyType.STATIC&&b.type==PhysicsV2BodyType.STATIC)continue;if(!a.filter.allows(b.filter))continue;val key=pair(a.id,b.id);val c=collide(a,b)?:continue;currentPairs+=key;contacts+=c;if(previousPairs.contains(key)){stayCount++;stayListeners.forEach{it(c)}}else{beginCount++;beginListeners.forEach{it(c)}};if(!c.sensor)resolve(a,b,c)}; joints.values.filter{it.def.bodyA in bodies&&it.def.bodyB in bodies}.forEach{solveJoint(it,dt)}; updateSleep(dt);for(k in previousPairs)if(k !in currentPairs){endCount++;endListeners.forEach{it(pairA(k),pairB(k))}}; simulationTime+=dt;stepCount++ }
    ...
    private fun pair(a:Int,b:Int):Long{val x=min(a,b).toLong();val y=max(a,b).toLong();return(x shl 32) xor(y and 0xffffffffL)}; private fun pairA(k:Long)=(k shr 32).toInt();private fun pairB(k:Long)=k.toInt()
}
class PhysicsV2BodyHandle internal constructor(private val world:PhysicsWorldV2,private val id:Int){
    fun id()=id; fun position():PhysicsV2Vec2=world.bodyState(id)?.position?.copy()?:PhysicsV2Vec2(); fun velocity():PhysicsV2Vec2=world.bodyState(id)?.velocity?.copy()?:PhysicsV2Vec2(); fun setVelocity(x:Float,y:Float){world.bodyState(id)?.velocity?.set(x,y)}; fun setPosition(x:Float,y:Float){world.bodyState(id)?.position?.set(x,y)}; fun applyForce(x:Float,y:Float){world.bodyState(id)?.let{if(it.dynamic()){it.force.x+=x;it.force.y+=y;it.wake()}}}; fun applyImpulse(x:Float,y:Float){world.bodyState(id)?.let{if(it.dynamic()){it.velocity.x+=x*it.invMass;it.velocity.y+=y*it.invMass;it.wake()}}}; fun setAwake(value:Boolean){world.bodyState(id)?.let{it.awake=value;if(value)it.wake()}}
}
